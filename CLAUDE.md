...
**ai_telco_release** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg)
, JWT + API Key auth, Redis, LangGraph, RAG (milvus), Taskiq, Next.js 15 (i18n)
...
# RAG
uv run ai_telco_release rag-collections
uv run ai_telco_release rag-ingest /path/to/file.pdf --collection docs
uv run ai_telco_release rag-search "query" --collection docs
uv run ai_telco_release rag-sync-gdrive --collection docs

# Sync Sources
uv run ai_telco_release cmd rag-sources
uv run ai_telco_release cmd rag-source-add
uv run ai_telco_release cmd rag-source-sync
```
