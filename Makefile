...
# `make prod`  — production (docker-compose.prod.yml — needs .env.prod + nginx)
...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app ai_telco_release db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		ai_telco_release user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app ai_telco_release db upgrade
	@echo "✅ Staging stack at http://localhost:8000"

stage-down:
	docker compose -f docker-compose.yml down

# === Production: external Nginx, .env.prod required, real secrets ===
prod:
	@test -f .env.prod || (echo "❌ .env.prod missing — copy from .env.prod.example and fill in values" && exit 1)
	docker compose -f docker-compose.prod.yml up -d --build
	@echo "▶ Waiting for DB then running migrations…"
	@sleep 5
	docker compose -f docker-compose.prod.yml exec -T app ai_telco_release db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run ai_telco_release db migrate -m "initial" || true
	cd backend && uv run ai_telco_release db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend ai_telco_release db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend ai_telco_release db upgrade

db-downgrade:
	uv run --directory backend ai_telco_release db downgrade

db-current:
	uv run --directory backend ai_telco_release db current

db-history:
	uv run --directory backend ai_telco_release db history

# === Server ===
run:
	uv run --directory backend ai_telco_release server run --reload

run-prod:
	uv run --directory backend ai_telco_release server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend ai_telco_release server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend ai_telco_release user create-admin

user-create:
	uv run --directory backend ai_telco_release user create

user-list:
	uv run --directory backend ai_telco_release user list

# === Taskiq ===
taskiq-worker:
	uv run --directory backend ai_telco_release taskiq worker

taskiq-scheduler:
	uv run --directory backend ai_telco_release taskiq scheduler
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/ai_telco_release"
...
help:
	@echo ""
	@echo "ai_telco_release - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo "  make prod           Production stack (requires .env.prod + nginx)"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run ai_telco_release rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run ai_telco_release rag-search <query> -c <collection>  Search"
	@echo "  uv run ai_telco_release rag-collections                     List collections"
	@echo "  uv run ai_telco_release rag-sources                         List sync sources"
	@echo "  uv run ai_telco_release rag-source-add                      Add sync source"
	@echo "  uv run ai_telco_release rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Taskiq:"
