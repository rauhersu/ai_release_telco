...
| **Cache** | Redis |
| **AI Framework** | langgraph (anthropic) |
...
└── commands/             # Click CLI commands (auto-discovered by `ai_telco_release cmd …`)
...
The generated project ships a Click CLI exposed as `ai_telco_release` (after `make install`):

```bash
ai_telco_release server run --reload          # dev server
ai_telco_release db upgrade                   # apply migrations
ai_telco_release db migrate -m "message"      # create new migration
ai_telco_release user create-admin            # interactive admin creation
ai_telco_release rag-ingest <path> -c docs    # ingest local files
ai_telco_release rag-search "query" -c docs   # semantic search
ai_telco_release rag-collections              # list collections
```

Run `make help` for a categorized list, or `ai_telco_release --help` for full CLI docs.
...
Using **milvus** as the vector store with **voyage** embeddings.

```bash
# Ingest local files (recursive)
ai_telco_release rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
ai_telco_release rag-sync-gdrive --collection documents --folder-id <id>

# Semantic search
ai_telco_release rag-search "your query" --collection documents
```
...
*Generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template) v0.2.8.*
