...
Run with: `uv run ai_telco_release cmd my-command --name test`
...
# Or use CLI
uv run ai_telco_release db migrate -m "Add notifications table"
uv run ai_telco_release db upgrade
```
